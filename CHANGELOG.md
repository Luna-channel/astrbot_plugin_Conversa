...
- 新增「主动消息离线保护」基础设置，默认开启。
- 同一平台的 Conversa 主动消息连续 3 次发送失败后，会进入该平台的离线保护状态，暂停该平台主动触发，避免端掉线时继续请求 LLM 浪费 API 资源。
- 离线保护只检测并打断 Conversa 自身主动触发，不修改 AstrBot 消息生命周期，不影响其他插件。
- 检测到真实用户消息或主动消息发送恢复后，自动解除离线保护。
